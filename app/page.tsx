...
function startOfWeekIso(offsetDays = 0) { const d = new Date(); d.setDate(d.getDate() + offsetDays); const day = d.getDay(); d.setDate(d.getDate() - day); return d.toISOString().slice(0, 10); }
function endFromStart(start: string, days: number) { const d = new Date(`${start}T12:00:00`); d.setDate(d.getDate() + days); return d.toISOString().slice(0, 10); }
...
export default function Home() {
  ...
  async function loadShared() {
    try {
      ...
      setState(data.state as AppState); setShared(Boolean(data.shared)); setSyncStatus(data.shared ? "Shared database connected" : "Testing mode: browser storage only");
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data.state));
    } catch { const saved = localStorage.getItem(STORAGE_KEY); if (saved) setState(JSON.parse(saved) as AppState); setSyncStatus("Offline fallback: browser storage only"); }
    finally { setHydrated(true); }
  }
  async function persist(next: AppState) {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(next)); setState(next);
    try { const res = await fetch("/api/state", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ state: next }) }); const data = await res.json(); if (data.ok) { setShared(Boolean(data.shared)); setSyncStatus(data.shared ? "Saved to shared database" : "Saved in testing mode"); setState(data.state as AppState); } }
  ...
  const byDate = useMemo(() => { const grouped = new Map<string, Shift[]>(); state.shifts.forEach((shift) => { if (!grouped.has(shift.date)) grouped.set(shift.date, []); grouped.get(shift.date)?.push(shift); }); return grouped; }, [state.shifts]);
  ...
  function submitRequests() { if (!selectedName || selected.length === 0) return; updateState((current) => { const existing = new Set(current.requests.map((r) => `${r.name.toLowerCase()}|${r.shiftId}`)); const additions: RequestItem[] = selected.filter((shiftId) => !existing.has(`${selectedName.toLowerCase()}|${shiftId}`)).map((shiftId) => ({ id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, name: selectedName, shiftId, status: "pending", createdAt: new Date().toISOString() })); return { ...current, requests: [...current.requests, ...additions] }; }); setSelected([]); setView("confirm"); }
  function adminLogin() { if (pin === ADMIN_CODE) { setAdminAuthed(true); setView("admin"); setPin(""); } }
  function approveRequest(req: RequestItem) { updateState((current) => { const shift = current.shifts.find((s) => s.id === req.shiftId); if (!shift) return current; const canAssign = openCount(shift) > 0 && !shift.assignments.some((a) => a.name.toLowerCase() === req.name.toLowerCase()) && !sameDayDouble(current.shifts, shift, req.name); return { ...current, shifts: current.shifts.map((s) => s.id === req.shiftId && canAssign ? { ...s, assignments: [...s.assignments, { name: req.name, source: "request" }] } : s), requests: current.requests.map((r) => r.id === req.id ? { ...r, status: canAssign ? "approved" : "rejected" } : r) }; }); }
  function rejectRequest(id: string) { updateState((current) => ({ ...current, requests: current.requests.map((r) => r.id === id ? { ...r, status: "rejected" } : r) })); }
  function addManual(shiftId: string) { const clean = manualName.trim(); if (!clean) return; updateState((current) => ({ ...current, shifts: current.shifts.map((s) => s.id === shiftId && openCount(s) > 0 && !s.assignments.some((a) => a.name.toLowerCase() === clean.toLowerCase()) ? { ...s, assignments: [...s.assignments, { name: clean, source: "admin" }] } : s) })); setManualName(""); }
  ...
  function saveEditedName() { if (!edit || !edit.value.trim()) return; updateState((current) => ({ ...current, shifts: current.shifts.map((s) => s.id === edit.shiftId ? { ...s, assignments: s.assignments.map((a) => a.name === edit.oldName ? { ...a, name: edit.value.trim() } : a) } : s) })); setEdit(null); }
  function resetAll() { if (resetText !== "RESET SCHEDULE") return; void persist(blankState()); setResetText(""); }
  function exportReport() { const rows = rowsBetween(state.shifts, reportStart, reportEnd); const csv = ["Serenity Shores Pool Schedule", `${longDate(reportStart)} through ${longDate(reportEnd)}`, "", "Date,AM 10-3:30,PM 3:30-10,Open Spots", ...rows.map((r) => [r.date, r.am, r.pm, String(r.open)].map(csvSafe).join(","))].join("\n"); const blob = new Blob([csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `serenity-shores-pool-schedule-${reportStart}-to-${reportEnd}.csv`; link.click(); URL.revokeObjectURL(url); }
  async function exportPdf() { const { PDFDocument, StandardFonts, rgb } = await import("pdf-lib"); const pdf = await PDFDocument.create(); const page = pdf.addPage([792, 612]); const font = await pdf.embedFont(StandardFonts.Helvetica); const bold = await pdf.embedFont(StandardFonts.HelveticaBold); page.drawText("Serenity Shores Pool Schedule", { x: 36, y: 566, size: 18, font: bold }); page.drawText(`${longDate(reportStart)} through ${longDate(reportEnd)}`, { x: 36, y: 546, size: 10, font }); let y = 510; const rows = rowsBetween(state.shifts, reportStart, reportEnd); const widths = [165, 250, 250, 55]; const xs = [36, 201, 451, 701]; ["Date", "AM 10-3:30", "PM 3:30-10", "Open"].forEach((h, i) => { page.drawRectangle({ x: xs[i], y, width: widths[i], height: 18, color: rgb(.9,.9,.9), borderColor: rgb(0,0,0), borderWidth: .5 }); page.drawText(h, { x: xs[i] + 4, y: y + 5, size: 8, font: bold }); }); y -= 18; rows.forEach((r) => { if (y < 44) return; const vals = [r.date, r.am, r.pm, String(r.open)]; vals.forEach((v, i) => { page.drawRectangle({ x: xs[i], y, width: widths[i], height: 28, borderColor: rgb(0,0,0), borderWidth: .35 }); page.drawText(v.slice(0, i === 0 ? 28 : 54), { x: xs[i] + 4, y: y + 16, size: 7, font }); }); y -= 28; }); const bytes = await pdf.save(); const blob = new Blob([bytes], { type: "application/pdf" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `serenity-shores-pool-schedule-${reportStart}-to-${reportEnd}.pdf`; link.click(); URL.revokeObjectURL(url); }
  function renderShiftButton(shift: Shift) { const requested = myRequests.some((r) => r.shiftId === shift.id && r.status !== "rejected"); const isSelected = selected.includes(shift.id); const isMine = requested || isSelected; if (filter === "open" && openCount(shift) <= 0) return null; if (filter === "mine" && !isMine) return null; return <button key={shift.id} className="shiftBtn" data-selected={isSelected} disabled={openCount(shift) <= 0 || requested} onClick={() => toggleShift(shift.id)}><span className="shiftTitle"><span>{shift.type === "AM" ? "Morning" : "Afternoon"}</span><span>{shift.start} - {shift.end}</span></span><span className="shiftMeta">{shift.assignments.length}/{shift.required} scheduled · {openCount(shift)} needed</span><span>{openCount(shift) > 0 ? <span className="badge badgeOpen">Open</span> : <span className="badge badgeFull">Full</span>} {requested ? <span className="badge badgePending">Requested</span> : null}</span></button>; }

  return <main className="appShell"><div className="topStrip">Serenity Shores pool · Lifeguard schedule</div><header className="header"><div className="brand"><img className="brandLogo" src="/serenity-shores-logo.svg" alt="Serenity Shores Table Rock Lake" /></div><button className="adminBtn" onClick={() => setView(adminAuthed ? "admin" : "adminPin")}>Admin</button></header><section className="main"><p className="small" style={{ marginTop: 0 }}>{statusText}</p>
