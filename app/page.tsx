...
export default function Home() {
  ...
  async function persist(nextInput: AppState, replace = false, hardReplace = false) {
    ...
    } catch { setSyncStatus(`Offline fallback: browser storage only · ${VERSION}`); }
  }
  ...
  function submitName() { const cleanName = selectedName; const cleanGuardPin = cleanPin(lifeguardPin); const match = state.lifeguards.find((g) => sameName(g.name, cleanName) && g.pin === cleanGuardPin); if (!cleanName || cleanGuardPin.length !== PIN_DIGITS) return setLoginError("Enter your first name and six digit PIN."); if (!match) return setLoginError("That name and PIN do not match an active lifeguard record. Ask Hollie/admin to add or correct your PIN."); setName(match.name); setLoginError(""); setView("select"); }
  function toggleShift(shiftId: string) { const shift = state.shifts.find((s) => s.id === shiftId); if (!shift || shift.date < today || shift.date > selectableEnd || openCount(shift) <= 0) return; setSelected((cur) => (cur.includes(shiftId) ? cur.filter((id) => id !== shiftId) : [...cur, shiftId])); }
  function submitAvailability() { if (!selectedName || !selected.length) return; updateState((cur) => { let nextRequests = cur.requests; selected.forEach((shiftId) => { const exists = nextRequests.some((r) => r.shiftId === shiftId && sameName(r.name, selectedName) && r.status !== "rejected"); if (!exists) nextRequests = upsertRequest(nextRequests, shiftId, selectedName, "pending"); }); return { ...cur, requests: nextRequests }; }); setSelected([]); setView("confirm"); }
