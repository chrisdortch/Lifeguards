...
export default function Home() {
  ...
  const [adminDate, setAdminDate] = useState(isoDate(new Date()));
  const [reportStart, setReportStart] = useState(isoDate(new Date()));
  const [reportEnd, setReportEnd] = useState(isoDate(new Date(Date.now() + 1000 * 60 * 60 * 24 * 14)));
  ...
  function submitRequests() {
    ...
    updateState((current) => {
      ...
      const additions = selected
        ...
        .map((shiftId) => ({ id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, name: selectedName, shiftId, status: "pending" as RequestStatus, createdAt: new Date().toISOString() }));
  ...
  function exportPdf() {
    ...
    doc.setFontSize(18);
    doc.text("Serenity Shores Pool Schedule", 40, 42);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.text(`${longDate(reportStart)} through ${longDate(reportEnd)}`, 40, 60);
    const rows = state.shifts
      ...
      .reduce<Record<string, { date: string; am: string; pm: string; open: number }>>((acc, s) => {
        ...
        acc[s.date][s.type === "AM" ? "am" : "pm"] = `${names} (${openCount(s)} open)`;
        acc[s.date].open += openCount(s);
        return acc;
      }, {});
    autoTable(doc, {
      head: [["Date", "AM 10-3:30", "PM 3:30-10", "Open Spots"]],
      body: Object.values(rows).map((r) => [r.date, r.am, r.pm, String(r.open)]),
      startY: 82,
      styles: { fontSize: 8, cellPadding: 5, lineColor: [0, 0, 0], lineWidth: .25, textColor: [0, 0, 0] },
      headStyles: { fillColor: [230, 230, 230], textColor: [0, 0, 0] }
    });
    doc.save(`serenity-shores-pool-schedule-${reportStart}-to-${reportEnd}.pdf`);
  }
  ...
  function renderShiftButton(shift: Shift) {
    ...
    return (
      <button key={shift.id} className="shiftBtn" data-selected={isSelected} disabled={openCount(shift) <= 0 || requested} onClick={() => toggleShift(shift.id)}>
        ...
        <span>{openCount(shift) > 0 ? <span className="badge badgeOpen">Open</span> : <span className="badge badgeFull">Full</span>} {requested && <span className="badge badgePending">Requested</span>}</span>
      </button>
    );
  }
  ...
  return (
    <main className="appShell">
      ...
      <header className="header">
        <div className="brand"><p className="brandTitle">Pool Schedule</p><p className="brandSub">Serenity Shores</p></div>
        <button className="adminBtn" onClick={() => setView(adminAuthed ? "admin" : "adminPin")}>Admin</button>
      </header>

      <section className="main">
        {view === "entry" && <div className="card hero stack">
          <span className="kicker">Lifeguard check-in</span>
          <h1>Help fill the pool schedule.</h1>
          <p className="lead">Enter your first name, choose the morning or afternoon shifts you can cover, then submit. Holly/admin approves the final schedule.</p>
        ...
        {view === "select" && <div className="stack">
          <div className="card stack">
            <div className="row"><div><h2>Hi, {selectedName}</h2><p className="small">Select openings you are available to cover.</p></div><button className="ghostBtn" onClick={() => setView("entry")}>Change</button></div>
            <div className="panelGrid"><div className="stat"><div className="statNum">{totalOpen}</div><div className="statLabel">Open spots</div></div><div className="stat"><div className="statNum">{selected.length}</div><div className="statLabel">Selected</div></div></div>
            <div className="tabs"><button className="tab" data-active={filter === "open"} onClick={() => setFilter("open")}>Open only</button><button className="tab" data-active={filter === "all"} onClick={() => setFilter("all")}>All shifts</button><button className="tab" data-active={filter === "mine"} onClick={() => setFilter("mine")}>Mine</button></div>
          </div>
          {visibleDates.map((date) => <div className="shiftCard" key={date}><div className="dateLine">{niceDate(date)}</div><div className="shiftGrid">{(byDate.get(date) || []).map(renderShiftButton)}</div></div>)}
          <div className="stickySubmit"><div className="stickySubmitInner"><button className="primaryBtn" disabled={selected.length === 0} onClick={submitRequests}>Submit {selected.length || ""} Shift Request{selected.length === 1 ? "" : "s"}</button><span className="small">Requests wait for admin approval before becoming final.</span></div></div>
        </div>}

        {view === "confirm" && <div className="card hero stack"><span className="kicker">Submitted</span><h1>Thank you.</h1><p className="lead">Your shift request was sent to the admin queue. The official schedule only changes after admin approval.</p><button className="primaryBtn" onClick={() => setView("select")}>Choose More Shifts</button></div>}

        {view === "adminPin" && <div className="card hero stack"><span className="kicker">Admin access</span><h1>Enter the code.</h1><input className="input" inputMode="numeric" placeholder="Admin code" value={pin} onChange={(e) => setPin(e.target.value)} onKeyDown={(e) => e.key === "Enter" && adminLogin()} /><button className="primaryBtn" onClick={adminLogin}>Open Admin</button><p className="small">Admin controls approvals, final schedule edits, reset, and PDF reports.</p></div>}

        {view === "admin" && <div className="stack">
          <div className="card stack">
            <span className="kicker">Admin dashboard</span><h2>Coverage control center</h2>
            <div className="panelGrid"><div className="stat"><div className="statNum">{pendingRequests.length}</div><div className="statLabel">Pending</div></div><div className="stat"><div className="statNum">{totalOpen}</div><div className="statLabel">Open spots</div></div><div className="stat"><div className="statNum">{filledSlots}</div><div className="statLabel">Scheduled</div></div><div className="stat"><div className="statNum">{openShifts.length}</div><div className="statLabel">Shifts with gaps</div></div></div>
          </div>

          <div className="card stack"><h3>Pending requests</h3>{pendingRequests.length === 0 && <p className="small">No pending requests right now.</p>}{pendingRequests.map((r) => { const s = state.shifts.find((x) => x.id === r.shiftId); return <div className="shiftCard" key={r.id}><div className="row"><div><strong>{r.name}</strong><div className="small">{s ? `${longDate(s.date)} · ${s.type} · ${s.start}-${s.end}` : r.shiftId}</div></div><span className="badge badgePending">Pending</span></div><div className="actions"><button className="secondaryBtn" onClick={() => approveRequest(r)}>Approve</button><button className="dangerBtn" onClick={() => rejectRequest(r.id)}>Reject</button></div></div>; })}</div>

          <div className="card stack"><h3>Schedule editor</h3><label className="small">Date</label><input className="input" type="date" value={adminDate} onChange={(e) => setAdminDate(e.target.value)} /><label className="small">Add lifeguard manually</label><input className="input" placeholder="Name to add" value={manualName} onChange={(e) => setManualName(e.target.value)} />{(byDate.get(adminDate) || []).map((s) => <div className="shiftCard" key={s.id}><div className="row"><div><strong>{s.type} · {s.start}-{s.end}</strong><div className="small">{openCount(s)} open spot{openCount(s) === 1 ? "" : "s"}</div></div>{openCount(s) > 0 ? <span className="badge badgeOpen">Needs help</span> : <span className="badge badgeFull">Full</span>}</div><div>{s.assignments.map((a) => <span className="namePill" key={`${s.id}-${a.name}`}>{a.name} <button className="noPrint" style={{ marginLeft: 6, border: 0, background: "transparent", fontWeight: 900 }} onClick={() => removeAssignment(s.id, a.name)}>×</button></span>) || null}</div><button className="secondaryBtn" disabled={!manualName.trim() || openCount(s) <= 0} onClick={() => addManual(s.id)}>Add to this shift</button></div>)}</div>

          <div className="card stack"><h3>PDF report</h3><div className="shiftGrid"><div><label className="small">Start date</label><input className="input" type="date" value={reportStart} onChange={(e) => setReportStart(e.target.value)} /></div><div><label className="small">End date</label><input className="input" type="date" value={reportEnd} onChange={(e) => setReportEnd(e.target.value)} /></div></div><button className="primaryBtn" onClick={exportPdf}>Download PDF Report</button></div>

          <div className="card stack"><h3>Danger zone</h3><p className="small">To reset all requests and schedule assignments, type RESET SCHEDULE exactly.</p><input className="input" placeholder="RESET SCHEDULE" value={resetText} onChange={(e) => setResetText(e.target.value)} /><button className="dangerBtn" disabled={resetText !== "RESET SCHEDULE"} onClick={resetAll}>Reset Everything</button></div>
        </div>}
      </section>
    </main>
  );
