...
import { AppState, blankState, buildInitialShifts } from "./schedule-v4";
import { getState as getStateV1 } from "./store";

const KEY = "v4-preview";
...
function mergeHistory(existing: AppState["history"] = [], incoming: AppState["history"] = []) {
  ...
  return Array.from(map.values()).sort((a, b) => b.at.localeCompare(a.at)).slice(0, 3000);
}
...
export async function getStateV4(): Promise<AppState> {
  const sql = getSql();
...
export async function saveStateV4(state: AppState, options: SaveOptions = {}): Promise<AppState> {
  const sql = getSql();
